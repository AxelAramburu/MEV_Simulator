...
export const swapQuote = async ():Promise<Amounts>  =>  {
  try {
    ...
    let estimatedAmountIn = `estimatedAmountIn: ${DecimalUtil.fromBN(quote.estimatedAmountIn, devUSDC.decimals).toString()} devUSDC`
    let estimatedAmountOut = `estimatedAmountOut: ${DecimalUtil.fromBN(quote.estimatedAmountOut, devSAMO.decimals).toString()} devSAMO`
